...
modDatetime: 2024-05-15T13:54:09Z
title: 面基记录
featured: false
draft: false
tags:
...
在 Apache RocketMQ 中，消息堆积主要发生在以下几个位置：

1. Broker 存储：
   - 当生产者发送消息到 Broker 时，如果消费者来不及消费，消息会被存储在 Broker 的磁盘上。RocketMQ 的消息存储主要在 ROCKETMQ_HOME/store 目录下的几个关键文件中：
     - commitLog：存储消息的实际内容。
     - consumeQueue：存储消息的索引，便于快速定位消息在 commitLog 中的位置。
     - indexFile：索引文件，用于快速查询消息。
2. Message Queue：
   - 消息会被分配到不同的 Message Queue 中，每个 Message Queue 对应一个消费者组的一个订阅实例。如果消费者的消费速度慢于生产者的发送速度，Message Queue 中的消息数量会增加，形成堆积。
3. Consumer 端：
   - 如果 Consumer 端由于某种原因（如故障、慢速处理、网络问题等）无法正常消费消息，那么这些消息也会在 Consumer 的内部队列中堆积。
4. 主题（Topic）级别：- 消息堆积可以是全局的，即整个 Topic 下的消息都堆积，也可以是特定的 Consumer Group 级别，即只有某个特定的 Consumer Group 消费缓慢导致堆积。
...
2. Spring Cloud LoadBalancer：- 时代背景：随着云原生和 Kubernetes 的普及，Spring Cloud 社区推出了 LoadBalancer，它更加现代化，旨在支持多种云服务的负载均衡需求。- 工作方式：Spring Cloud LoadBalancer 是一个更抽象的层，它支持多种负载均衡实现，包括基于 Kubernetes 的服务发现。它通过 Spring 的 @LoadBalanced 注解与 RestTemplate 或 WebClient 结合使用，或者与 OpenFeign 结合。- 配置：LoadBalancer 通过 Spring Boot 的配置方式，不需要在客户端代码中进行过多的定制，它可以自动与服务发现组件（如 Eureka、Consul、Kubernetes 等）集成。- 优势：提供了更灵活的扩展性，支持更多负载均衡策略，更容易适应云环境的变化。
...
8. 适配器模式（Adapter Pattern）：- 适配器模式可能体现在 OpenFeign 将 HTTP 请求转换为接口调用，以及将接口调用的结果转换为 HTTP 响应的过程中，适配不同的网络通信协议。
...
7. 支持负载均衡和服务发现 - 在负载均衡和服务发现的场景下，代理类还负责选择合适的服务实例，并发送请求到选择的服务实例。代理类会根据负载均衡策略和服务发现机制来选择服务实例，并将请求转发到选中的服务实例上。
...
5. **系统资源限制**：- 考虑到内存限制，如果每个线程占用大量内存，线程池的大小不能无限制增加，以免超出系统内存承受范围，导致性能下降甚至系统崩溃。
...
5. 命名约定：- 如果 @Resource 的 name 属性没有设置，Spring 会尝试使用目标字段或方法的名称作为 Bean 的名称来查找。
...
从 10,000 个数中选取 10 个数，最好的算法通常是组合算法。组合算法可以确保你选择的这 10 个数不重复，并且可以在保持一定效率的情况下遍历所有可能的组合。

常见的组合算法包括回溯法、动态规划和递归等。在这种情况下，你可以考虑使用组合算法来解决问题。

如果你只是想要随机选择 10 个数，那么随机选择算法也是一个不错的选择。Java 中的 Random 类可以帮助你实现随机选择。

对于从 10,000 个数中选出最小的 10 个数，通常可以使用堆排序算法。堆排序算法利用了堆这种数据结构，具有较好的时间复杂度，可以在 O(n log n) 的时间内完成排序。

堆排序的基本思想是先构建一个最小堆（或最大堆），然后依次将堆顶元素取出，这样就可以得到从小到大（或从大到小）排列的结果。在这种情况下，我们只需构建一个最小堆，并依次取出堆顶的 10 个元素即可。

Java 中的 PriorityQueue 类实现了堆结构，因此可以很方便地使用它来实现从 10,000 个数中取出最小的 10 个数。
